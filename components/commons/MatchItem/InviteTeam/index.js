...
export default function InviteTeam({show, setShow, match}) {
    ...
    useEffect(()=>{
        axios.get(TEAM_SPORT + match.sport, {
            headers:{
                ...
                <div className={styles.list}>
                    {list.map((element, key) => {
                        return (
                            ...
                            < Item key={key} id={element.id} name={element.name} match_id={match.id} avatar={element.avatar} ></Item>
                            // </LazyLoad>
                        )
                    })}
                </div>
